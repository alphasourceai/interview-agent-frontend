...
| Deployment | Subdomain Transition — Create and configure new subdomain `interviews.alphasourceai.com` for direct interview access (bypassing Wix embed) | ✅ Completed — Now live and validated across Safari, Chrome, Edge, iOS; camera/mic permissions confirmed. | Oct 28 |
...
## 🚫 Cancelled
| Area | Task | Notes | Owner | Date Cancelled |
|------|------|--------|--------|----------------|
| Wix Integration | 🔗 Embed cleanup, scaling, and token routing improvements for Admin and Account pages | Cancelled — Interview page successfully transitioned to standalone subdomain; Wix no longer required. Future embeds will be reconsidered in WordPress migration (Phase 2). | Jason | Oct 29 |
